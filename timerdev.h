...
class TimerDev : public Device
{
    Q_OBJECT

public:
    explicit TimerDev(QObject *parent = 0);

    TimerDev(const QString name, u_int32_t base) : Device(name, base) {}
...
private:
    void fill_ptv_details(const u_int32_t &value, const u_int32_t &new_value);
    void fill_pcr_details(const u_int32_t &value, const u_int32_t &new_value);
